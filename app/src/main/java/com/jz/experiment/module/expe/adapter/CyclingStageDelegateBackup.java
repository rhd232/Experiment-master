...
public class CyclingStageDelegateBackup extends BaseAdapterDelegate<CyclingStageDelegateBackup.ViewHolder> {

    List<VernierAdapter> vernierAdapters;
    public CyclingStageDelegateBackup(Activity activity, int layoutRes) {
        super(activity, layoutRes);
        vernierAdapters=new ArrayList<>();
    }
    ...
    protected void onBindViewHolder(@NonNull List<DisplayItem> items, final int position,
        ...
        if(((ViewHolder) holder).rvInner.getAdapter()==null){
            VernierAdapter adapter = new VernierAdapter(mActivity, R.layout.item_inner_vernier);
            vh.rvInner.setAdapter(adapter);
            adapter.replace(new PartStage());

        }
        //stage.setVernierAdapter((VernierAdapter) vh.rvInner.getAdapter());
